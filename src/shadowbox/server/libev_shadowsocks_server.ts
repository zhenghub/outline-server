...
export interface ShadowsocksMetrics {
  // addInboundBytes only reports inbound bytes, received from the client or from the target.
  //
  ...
  addInboundBytes(accessKey: string, countryCode: string, bytesTransferred: number);
}

export function createShadowsocksMetrics(registry: prometheus.Registry): ShadowsocksMetrics {
  ...
  return {
    addInboundBytes(accessKey: string, countryCode: string, inboundBytes: number) {
      usageCounter.labels('>p<', '', countryCode, '', accessKey).inc(inboundBytes);
    }
  };
}
...
export class LibevShadowsocksServer implements ShadowsocksServer {
  ...
  constructor(
      private publicAddress: string, private metrics: ShadowsocksMetrics,
      private metricsSocket: dgram.Socket, private verbose: boolean) {
    metricsSocket.on('message', (buf: Buffer) => {
      ...
      getConnectedClientIPAddresses(metricsMessage.portNumber)
          .then((ipAddresses: string[]) => {
            // TODO: Get Countries from IP addresses.
            this.metrics.addInboundBytes(
                this.portId[metricsMessage.portNumber] || '', 'ZZ', dataDelta);
          })
          .catch((err) => {
